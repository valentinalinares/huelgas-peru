...
def normalize_validation_sheet(path: Path, year: int) -> pd.DataFrame:
    validation = pd.read_excel(path, sheet_name="validacion")
    validation_columns = ["anio", "modulo", "hoja_excel", "metrica", "total_extraido", "total_fuente", "diferencia", "estado"]
    if set(validation_columns).issubset(validation.columns):
        return validation[validation_columns].copy()

    metric_map = {
        "huelgas": "huelgas",
        "trabajadores": "trabajadores_comprendidos",
        "hhp": "horas_hombre_perdidas",
    }
    rows: list[dict[str, object]] = []
    for _, row in validation.iterrows():
        check = str(row.get("check", "")).strip().lower()
        match = re.match(r"total_(huelgas|trabajadores|hhp)_(.+)$", check)
        if not match:
            continue
        metric_token, modulo = match.groups()
        total_extraido = pd.to_numeric(row.get("valor_obtenido"), errors="coerce")
        total_fuente = pd.to_numeric(row.get("valor_esperado"), errors="coerce")
        diferencia = None if pd.isna(total_extraido) or pd.isna(total_fuente) else total_extraido - total_fuente
        rows.append(
            {
                "anio": year,
                "modulo": modulo,
                "hoja_excel": None,
                "metrica": metric_map[metric_token],
                "total_extraido": None if pd.isna(total_extraido) else total_extraido,
                "total_fuente": None if pd.isna(total_fuente) else total_fuente,
                "diferencia": diferencia,
                "estado": str(row.get("ok", "")).strip().lower(),
            }
        )
    return pd.DataFrame(
        rows,
...
def metric_sum(df: pd.DataFrame, metric: str) -> float | None:
    if metric not in df.columns:
        return None
    total = pd.to_numeric(df[metric], errors="coerce").sum(min_count=1)
...
def augment_validation(
    ...
    metrics = ["huelgas", "trabajadores_comprendidos", "horas_hombre_perdidas"]
    if validation.empty:
        validation = pd.DataFrame(
            columns=["anio", "modulo", "hoja_excel", "metrica", "total_extraido", "total_fuente", "diferencia", "estado"]
        )

    reference = {metric: metric_sum(module_frames.get("actividad", pd.DataFrame()), metric) for metric in metrics}
    rows: list[dict[str, object]] = []
    existing = set(zip(validation["modulo"].astype(str), validation["metrica"].astype(str)))

    for modulo, module_df in module_frames.items():
        if module_df.empty:
            continue
        year = int(module_df["anio"].iloc[0])
        sheet = module_df["hoja_excel"].iloc[0] if "hoja_excel" in module_df.columns else None

        if modulo == "territorio":
            for metric in metrics:
                for suffix, level in [("todos_los_niveles", None), ("regional", "regional"), ("zona", "zona")]:
                    metric_name = f"{metric}_{suffix}"
                    if (modulo, metric_name) in existing:
                        continue
                    source_value = reference.get(metric)
                    if level is None:
                        extracted = metric_sum(module_df, metric)
                        state = "estructura_mixta"
                    else:
                        extracted = metric_sum(module_df[module_df["nivel_territorial"].eq(level)], metric)
                        diff = None if extracted is None or source_value is None else extracted - source_value
        ...
        for metric in metrics:
            ...
            if extracted is None:
                ...
                diff = None if source_value is None else extracted - source_value
                state = "ok" if diff is not None and abs(diff) <= 0.01 else "revisar"
            rows.append(
                {
                    "anio": year,
    ...
    if rows:
        validation = pd.concat([validation, pd.DataFrame(rows)], ignore_index=True)
    return validation
...
def collect_outputs() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for year, block, path in workbook_paths():
        ...
        validation = normalize_validation_sheet(path, year)
        ...
        module_frames_by_name: dict[str, pd.DataFrame] = {}
        for modulo in MODULES:
            ...
            module_frames_by_name[modulo] = module_df

        validation = augment_validation(validation, module_frames_by_name)
        validation["archivo_homologado"] = path.name
        validation["carpeta_bloque"] = block
        validation_frames.append(validation)

        for modulo, module_df in module_frames_by_name.items():
            estados = sorted(
...
def save_excel_bundle(
    ...
    output_path = OUTPUT_DIR / "huelgas_maestra_1993_2025.xlsx"
        ...
        "cells": [
            ...
            },
            {
                ...
                "source": [
                    ...
                    "GRAPH = BASE / 'graficos'\n",
                    "\n",
                    "master = pd.read_csv(BASE / 'huelgas_modulos_maestra_1993_2025.csv')\n",
                    "coverage = pd.read_csv(BASE / 'cobertura_modulos_1993_2025.csv')\n",
                    "legalidad = pd.read_csv(BASE / 'huelgas_legalidad_1996_2025_largo.csv')\n",
                    "legalidad_resumen = pd.read_csv(BASE / 'huelgas_legalidad_1996_2025_resumen_anual.csv')\n",
                    "verif = pd.read_csv(BASE / 'verificacion_calificacion_1993_2025_largo.csv')\n",
                    "verif_resumen = pd.read_csv(BASE / 'verificacion_calificacion_1993_2025_resumen.csv')\n",
...
def main() -> None:
    ...
    master_df.to_csv(OUTPUT_DIR / "huelgas_modulos_maestra_1993_2025.csv", index=False, encoding="utf-8-sig")
    coverage_df.to_csv(OUTPUT_DIR / "cobertura_modulos_1993_2025.csv", index=False, encoding="utf-8-sig")
    annual_summary_df.to_csv(OUTPUT_DIR / "resumen_anual_1993_2025.csv", index=False, encoding="utf-8-sig")
    common_language_detail.to_csv(OUTPUT_DIR / "diccionario_idioma_comun_1993_2025.csv", index=False, encoding="utf-8-sig")
    ...
    legalidad_long.to_csv(OUTPUT_DIR / "huelgas_legalidad_1996_2025_largo.csv", index=False, encoding="utf-8-sig")
    legalidad_resumen.to_csv(OUTPUT_DIR / "huelgas_legalidad_1996_2025_resumen_anual.csv", index=False, encoding="utf-8-sig")
    verif_long.to_csv(OUTPUT_DIR / "verificacion_calificacion_1993_2025_largo.csv", index=False, encoding="utf-8-sig")
    verif_resumen.to_csv(OUTPUT_DIR / "verificacion_calificacion_1993_2025_resumen.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"[ok] base maestra -> {OUTPUT_DIR / 'huelgas_modulos_maestra_1993_2025.csv'}")
    print(f"[ok] idioma comun -> {OUTPUT_DIR / 'diccionario_idioma_comun_1993_2025.csv'}")
    print(f"[ok] legalidad -> {OUTPUT_DIR / 'huelgas_legalidad_1996_2025_largo.csv'}")
    print(f"[ok] verificacion calificacion -> {OUTPUT_DIR / 'verificacion_calificacion_1993_2025_resumen.csv'}")
